...
public class GrowthYandereAlly extends YandereAlly {
    ...
    private String warningGrowthLine(int tier, int band) {
        ...
        if (tier == 3) {
            if (band >= 4) return pick(
                    "나 보라고 했잖아! 지금 나부터 봐! 나 진짜 한계야!",
                    "하트 줘. 지금. 더 기다리게 하면 나 진짜 못 참아!",
                    "좋아해 좋아해 좋아하는데 왜 이렇게 화나게 해! 지금 나 봐!",
        ...
        if (tier == 2) {
            if (band >= 4) return pick(
                    "나 두고 혼자 결정하지 마! 나 지금 진짜 한계야!",
                    "지금 나부터 챙겨! 더 기다리면 나도 내가 어떻게 될지 몰라.",
                    "네가 나 필요 없다는 생각 더 들게 하지 마. 지금 붙잡아줘!");
            return pick(
        ...
        if (tier == 1) {
            if (band >= 4) return pick(
                    "왜 나만 계속 기다리게 해! 지금은 나부터 봐!",
                    "내가 네 옆자리라고 했잖아! 나 진짜 더는 못 참아.",
                    "네 관심 내가 달라고! 지금 다른 거 보지 마!");
            return pick(
